...
<template>
        ...
        <div class="w-80 border-r">
          ...
          <ul class="list-disc pl-7 w-56">
            <li v-for="(r, i) in resep" :key="i" class="my-4 pb-1 border-b group">
              <a href="" @click.prevent class="transition duration-300 group-hover:-translate-y-0.5 group-hover:text-primary block" :class="r.active ? 'font-semibold text-primary' : ''">{{ r.title }}</a>
            </li>
          </ul>
        </div>
  ...
  <section id="makanan" class="py-24">
    <div class="container">
      ...
      <div class="grid grid-cols-3 gap-16 px-8 mb-36">
        <card-component v-for="data in makanan" :data="data" />
      </div>
      
      <h2 class="text-2xl font-bold w-max text-gray-700 border-l-4 border-primary pl-5 mb-20">Minuman</h2>
      
      <div class="grid grid-cols-3 gap-16 px-8 mb-36">
        <card-component v-for="dt in minuman" :data="dt" />
      </div>
    </div>
  </section>
</template>